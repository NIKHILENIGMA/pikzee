...
- **WebhookValidator:** A service responsible for validating the signatures of incoming webhooks to ensure their authenticity.
...
5.  The `WebhookService` calls the `WebhookValidator.validateSignature()` method to verify the webhook's signature.
...
5.  **Register the Handler:** In `server/src/modules/webhooks/webhook.module.ts`, instantiate your new handler and add it to the `handlers` map.
...
-   `webhook-validator.ts`: Contains the logic for validating incoming webhooks.
